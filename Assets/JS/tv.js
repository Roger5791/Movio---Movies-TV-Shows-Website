...
const heroBanner = function ({ results: movieList }) {
  ...
  for (const [index, tv] of movieList.entries()) {
    const {
      backdrop_path,
      title,
      name,
    ...
    sliderItem.innerHTML = `
      ...
      <div class="banner-content">
        ...
        <a href="./tv-detail.html" class="btn" onclick="getMovieDetail(${id})">
          <img
...
const createMovieList = function ({ results: movieList }, title) {
  ...
  for (const tv of movieList) {
    // Called from movie_card.js
    const movieCard = createMovieCard(tv);
...
$(document).ready(function() {
    ...
    function autoScroll () {
      ...
      scrollHandler = setInterval(function() {
        var nextScroll = $('.slider-control').scrollLeft() + 110;
        $('.slider-control').scrollLeft(nextScroll);
      },5000);
    }
